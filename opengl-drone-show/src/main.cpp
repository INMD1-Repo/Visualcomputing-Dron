...
Vec3 normalize(Vec3 v) {
    ...
    if (mag > 0.0001f) return v * (1.0f / mag);
    return {0, 0, 0};
}
...
Mat4 identity() { Mat4 mat; mat.m[0] = 1.0f; mat.m[5] = 1.0f; mat.m[10] = 1.0f; mat.m[15] = 1.0f; return mat; }
Mat4 perspective(float fov, float aspect, float n, float f) { Mat4 mat; float t = tan(fov/2.0f); mat.m[0]=1.0f/(aspect*t); mat.m[5]=1.0f/t; mat.m[10]=-(f+n)/(f-n); mat.m[11]=-1.0f; mat.m[14]=-(2.0f*f*n)/(f-n); return mat; }
Mat4 orthographic(float l, float r, float b, float t, float n, float f) { Mat4 mat=identity(); mat.m[0]=2.0f/(r-l); mat.m[5]=2.0f/(t-b); mat.m[10]=-2.0f/(f-n); mat.m[12]=-(r+l)/(r-l); mat.m[13]=-(t+b)/(t-b); mat.m[14]=-(f+n)/(f-n); return mat; }
...
float cameraYaw = -PI/2.0f, cameraPitch = 0.0f, cameraRadius = 500.0f; // For 3D orbit
...
void loadDroneShow(const char* path) {
    ...
    cJSON* layers = cJSON_GetObjectItem(root, "layers"); cJSON* layer;
    cJSON_ArrayForEach(layer, layers) {
        ...
        cJSON* points = cJSON_GetObjectItem(layer, "points"); cJSON* point;
...
void renderUI() {
    ImGui::SetNextWindowPos(ImVec2(0, 0)); ImGui::SetNextWindowSize(ImVec2(ImGui::GetIO().DisplaySize.x, 50));
    ImGui::Begin("Header", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove);
    ImGui::Text("%s", droneShow.title.c_str()); ImGui::End();

    ImGui::SetNextWindowPos(ImVec2(0, ImGui::GetIO().DisplaySize.y - 80)); ImGui::SetNextWindowSize(ImVec2(ImGui::GetIO().DisplaySize.x, 80));
    ImGui::Begin("Controls", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove);
    if (ImGui::Button(isPlaying ? "Pause" : "Play")) { isPlaying = !isPlaying; } ImGui::SameLine();
    ImGui::SetNextItemWidth(ImGui::GetWindowWidth() - 250);
    if (ImGui::SliderFloat("##timeline", &timelinePosition, 0.0f, 1.0f)) { elapsedTime = timelinePosition * totalDuration; } ImGui::SameLine();
    ImGui::SetNextItemWidth(120);
    ImGui::SliderFloat("Speed", &playbackSpeed, 0.1f, 4.0f, "%.1fx");
    ImGui::End();

    ImGui::SetNextWindowPos(ImVec2(10, 60)); ImGui::SetNextWindowSize(ImVec2(250, 350));
    ...
    if (ImGui::RadioButton("2D Top", currentViewMode == VIEW_2D_TOP)) { currentViewMode = VIEW_2D_TOP; } ImGui::SameLine();
    if (ImGui::RadioButton("2D Front", currentViewMode == VIEW_2D_FRONT)) { currentViewMode = VIEW_2D_FRONT; }
    ImGui::Separator();
    ...
    ImGui::SetNextWindowPos(ImVec2(ImGui::GetIO().DisplaySize.x - 260, 60)); ImGui::SetNextWindowSize(ImVec2(250, 200));
    ImGui::Begin("Layers");
    for (size_t i = 0; i < droneShow.layers.size(); ++i) {
        if (ImGui::Selectable(droneShow.layers[i].name.c_str(), currentLayer == (int)i)) {
            if (currentLayer != (int)i) triggerTransition((int)i);
        }
    }
    ImGui::End();
}
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 720, "OpenGL Drone Show", NULL, NULL);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!vertexData.empty()) {
            ...
            switch(currentViewMode) {
                case VIEW_3D:
                    camPos.x = cameraTarget.x + cameraRadius * cos(cameraPitch) * cos(cameraYaw);
                    camPos.y = cameraTarget.y + cameraRadius * sin(cameraPitch);
                    camPos.z = cameraTarget.z + cameraRadius * cos(cameraPitch) * sin(cameraYaw);
                    projection = perspective(45.0f, aspect, 0.1f, 5000.0f);
                    view = lookAt(camPos, cameraTarget, {0, 1, 0});
                    break;
                case VIEW_2D_TOP:
                    projection = orthographic(-orthoSize * aspect, orthoSize * aspect, -orthoSize, orthoSize, -1000.0f, 1000.0f);
                    view = lookAt({cameraTarget.x, 500, cameraTarget.z}, cameraTarget, {0, 0, -1});
                    break;
                case VIEW_2D_FRONT:
                    projection = orthographic(-orthoSize * aspect, orthoSize * aspect, -orthoSize, orthoSize, -1000.0f, 5000.0f);
